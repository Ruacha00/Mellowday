...
配置来源（均位于运行时数据目录或当前工作目录，默认不存在时不启用 MCP）：
- 全局配置：`data_dir()/mcp.json`
- 项目配置：`./.mellowday/mcp.json`
...
from mellowday.runtime.events import print_error, print_info
...
class McpConnection:
    ...
    async def initialize(self) -> None:
        ...
        await self._send_request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "mellowday", "version": "1.0.0"},
        })
...
class McpManager:
    ...
    无配置时 load_and_connect() 立即返回，不阻塞启动，也不抛异常。
    ...
    def _load_configs(self) -> dict[str, dict]:
        ...
        # 1. 全局配置：<data_dir>/mcp.json
        global_path = paths.data_dir() / "mcp.json"
        self._merge_config_file(global_path, merged)

        # 2. 当前项目配置：<cwd>/.mellowday/mcp.json
        project_path = Path.cwd() / ".mellowday" / "mcp.json"
