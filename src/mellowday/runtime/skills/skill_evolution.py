...
def resolve_skill_file(skill_name: str, *, target: str = "active", active_dir: str = "") -> Path | None:
    ...
    # 只有单一技能根目录：project / user / active 都解析到 <MELLOWDAY_DATA_DIR>/skills。
    if target in ("project", "user", "active"):
        found = _find_skill_file_by_name(paths.skills_dir(), skill_name)
        if found:
            return found
...
def _skills_root(target: str) -> Path:
    # target 仅保留在签名中用于兼容调用方；所有技能都落在同一个根目录。
...
def evolve_skill_file(
    *,
    ...
    if instructions.strip():
        ...
        #调用方传进来的正文可能已经带着既有的 ## Evolution Notes（合并/编辑路径会保留
        #溯源）。这时新条目要追加进**同一小节**，而不是再拼一个同名小节；同一条 lesson
        #重复出现时也不重复追加（_append_evolution_note 自带去重）。
        new_body = _append_evolution_note(new_body, lesson, rationale)
    else:
...
def _maybe_prune_stale_skill(skill_name: str, stats: dict[str, Any]) -> bool:
    min_retrieved = _parse_int(os.environ.get("MELLOWDAY_SKILL_USAGE_PRUNE_MIN_RETRIEVED"), 40)
    max_used = _parse_int(os.environ.get("MELLOWDAY_SKILL_USAGE_PRUNE_MAX_USED"), 0)
    source = str(stats.get("source") or "").strip().lower()
    if source != "user" and os.environ.get("MELLOWDAY_SKILL_PRUNE_PROJECT", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return False
    ...
    if skill_dir.name.startswith("."):
        return False
    archive_root = paths.skills_archive_dir()
