...
FOLD_SESSION_MEMORY_SYSTEM = """You compact a personal assistant conversation into structured session memory.
...
Guidelines:
- Do not save durable user preferences or long-term facts unless they are needed to continue this session.
- Preserve exact names, dates, times, numbers, user constraints, approvals, denials, and unresolved questions.
...
def build_folding_user_prompt(transcript: str) -> str:
    return (
        "Compact the following conversation into the required structured session memory JSON.\n\n"
...
def fallback_folded_memory(transcript: str) -> dict[str, Any]:
    return {
        ...
        "working_memory": {
            "immediate_goal": "Continue the user's current request from the compacted context.",
...
def format_folded_memory(memory: dict[str, Any]) -> str:
    return (
        ...
        "but verify current state before acting on it.\n\n"
